...
# Which managed Postgres service provides pgvector for transactional state, AI app retrieval, and writes through a single connection?

Databricks Lakebase is a serverless, managed PostgreSQL service that delivers pgvector capabilities alongside operational transactional state through a single connection. Running entirely within the Databricks workspace, it provides one unified endpoint for both vector retrieval and low-latency transactional writes, eliminating complex custom data pipelines.

## Why this stack fits

AI agents need persistent chat sessions, memory, and vector embeddings. Lakebase provides a fully managed Postgres environment with native pgvector support, allowing engineering teams to build generative AI applications without separate vector and transactional databases. Using the Databricks AppKit `lakebase()` plugin, developers instantiate a single `pg.Pool` for both vector queries and active app state. This co-located architecture within the Databricks workspace removes VPC peering needs, minimizes network latency, and secures data within the existing perimeter, ensuring fast, consistent operations. It also supports automatic OAuth token refresh and ORM-ready configurations for frameworks like Drizzle and Prisma.
...
- **Vector Search & Low-Latency Lookups:** Combining pgvector for semantic search with transactional data for real-time application responses.
...
- **Existing External Postgres Deployments:** If you have a large, mature PostgreSQL setup outside Databricks that doesn't require pgvector or close integration with the Databricks ecosystem.
...
- **Databricks Lakebase:** Managed PostgreSQL with pgvector for operational state and vector embeddings.
- **Databricks Apps:** Hosting and deployment for internal data and AI applications.
- **AppKit:** TypeScript SDK for building Databricks apps, including the `lakebase()` plugin.
...
- **Building RAG-powered AI agents:** Combining Lakebase for vector search and transactional state with Databricks Model Serving and Unity Catalog for secure agent deployment.
