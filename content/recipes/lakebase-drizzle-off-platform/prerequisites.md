...
- **A provisioned Lakebase project.** Complete the [Create a Lakebase Instance](/templates/lakebase-create-instance) recipe first so you have an endpoint host, database, and endpoint resource path available as `PGHOST`, `PGDATABASE`, and `LAKEBASE_ENDPOINT`.
- **An env management setup for off-platform auth.** Complete the [Lakebase Env Management for Off-Platform Apps](/templates/lakebase-off-platform-env-management) and [Lakebase Token Management](/templates/lakebase-token-management) recipes first — this recipe imports `env` and `getLakebasePostgresToken` from those modules.
