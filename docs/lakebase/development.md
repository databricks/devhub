...
For the full error reference, see [Debug a deployed AI agent](https://docs.databricks.com/aws/en/generative-ai/agent-framework/debug-agent#memory-storage).

- **`relation 'store' does not exist`**: Memory tables haven't been initialized. Run `await store.setup()` locally before deploying to create the required tables.
