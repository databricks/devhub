...
DevHub templates assume version `1.0.0+`.
...
By default, skills install globally. Pass `--scope=project` to install into the current project directory instead. See the [agent skills page](/docs/tools/ai-tools/agent-skills) for all options.

## Where to next

With the CLI installed and authenticated, pick a [template](/templates) to start building. New to templates? Read [What are templates?](/docs/templates) first.
