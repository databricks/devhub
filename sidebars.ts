...
// When only "latest" exists, show items directly without a version wrapper.
// When multiple channels exist (future), show each as a collapsible category.
const hasMultipleChannels = appKitChannels.length > 1;

const appKitVersionItems = hasMultipleChannels
	? appKitChannels
			.map((channel) => {
...
// For the single-channel case, read the tree directly
const latestChannel = appKitChannels[0];
const latestTree = latestChannel
	? readAppKitDocTree(path.join("appkit", latestChannel))
	: { indexDocId: null, items: [] };

const appKitItems = hasMultipleChannels ? appKitVersionItems : latestTree.items;

const latestAppKitDocId = latestTree.indexDocId;
