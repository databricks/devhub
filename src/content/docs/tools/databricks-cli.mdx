...
`brew trust databricks/tap` is required as of [Homebrew 6.0.0](https://docs.brew.sh/Tap-Trust) — including for existing users who tapped before upgrading.
...
Once authenticated, the CLI (v1.0.0+) stores your login token in your operating system's secure storage — Keychain on macOS, Credential Manager on Windows, or the Secret Service on Linux — not in `~/.databrickscfg`. See [Token storage](https://docs.databricks.com/aws/en/dev-tools/cli/authentication#auth-storage) for details and the plaintext fallback.
