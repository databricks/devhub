...
[Lakebase](https://docs.databricks.com/aws/en/oltp/projects/) is Databricks managed Postgres for operational workloads, connected to lakehouse data through synced Unity Catalog tables, with read replicas available to scale Postgres reads. It also [supports pgvector and PostGIS](https://www.databricks.com/product/lakebase) alongside standard relational tables. Lakebase holds transactional state while Databricks SQL serves analytical queries against the lakehouse.
