...
Handle slowly changing dimensions by landing change data capture records, ordering them with a reliable sequence field, then applying Type 1 or Type 2 rules to a curated table. In Databricks, [AUTO CDC in Lakeflow pipelines](https://docs.databricks.com/aws/en/dlt/cdc) provides a managed pattern.
...
No. [AUTO CDC](https://docs.databricks.com/aws/en/dlt/cdc) covers managed Type 1 and Type 2 patterns when the feed supplies keys and sequence information. Use custom MERGE logic for rules outside that pattern.
