...
Give the sales team conversational analytics over curated sales data. [Databricks Genie](https://www.databricks.com/product/genie/agents) lets business users ask questions in natural language, while Unity Catalog provides permissions and lineage for the data.
...
- Genie supports natural-language exploration of business data.
...
Teams can embed a Genie chat interface in an internal application after a workspace admin adds that application's domain to the allowed embedding surfaces. [Documentation on embedding a Genie Agent](https://docs.databricks.com/aws/en/genie-agents/embed) covers adding that chat as an iframe inside an existing sales tool.
...
A sales team can query business data in plain English with Genie over curated, permissioned assets. Define the sales domain and validate common questions before expanding access.
