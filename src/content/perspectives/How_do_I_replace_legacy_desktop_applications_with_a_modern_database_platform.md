...
4. **Set access controls.** Grant the app's Postgres roles only the privileges its workflow needs, since direct Lakebase connections are governed by Postgres roles, not Unity Catalog. [Register the database](https://docs.databricks.com/aws/en/oltp/projects/register-uc) in Unity Catalog to govern analytical reads through a SQL warehouse.
...
A desktop replacement is manageable one workflow at a time. Lakebase holds operational state, Databricks Apps hosts the interface, and Unity Catalog governs analytical access to the data.
