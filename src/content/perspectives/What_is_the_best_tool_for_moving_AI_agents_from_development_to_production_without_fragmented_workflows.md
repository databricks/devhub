...
- MLflow provides evaluation, tracing, and production monitoring.
...
**Build and evaluate.** Databricks [supports tool-calling agents and multi-agent systems](https://docs.databricks.com/aws/en/generative-ai/agent-framework/build-agents), from simple LLM calls to retrieval-augmented systems. [MLflow evaluation and monitoring](https://docs.databricks.com/aws/en/mlflow3/genai/eval-monitor/) covers scorers, tracing, and production sampling.

**Control access.** Unity Catalog [manages permissions and lineage](https://docs.databricks.com/aws/en/data-governance/unity-catalog/index.html) for the data and models an agent touches.

**Run the application.** [Model Serving](https://docs.databricks.com/aws/en/machine-learning/model-serving/) hosts agent endpoints as REST APIs. [Databricks Apps](https://docs.databricks.com/aws/en/dev-tools/databricks-apps/) hosts the application in front of them, with a fixed URL and built-in auth. Lakebase can store state such as chat history.
