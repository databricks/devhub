...
For data engineering and AI teams that need one governance model across their assets, the answer is Databricks Unity Catalog. It governs data, models, tools, and agents with permissions and lineage, extending lineage into dashboards built on that data.
...
Unity Catalog governs data, model, and tool access on Databricks, and provides lineage and discovery for those assets, extending lineage into dashboards built on them. Dashboard, notebook, and job permissions are managed through workspace access-control lists, a separate system.

**Can data engineering and AI teams use the same governance model?**

Yes. The model spans engineering data assets and the models and agents used by AI teams.

**Does one Unity Catalog metastore cover every cloud and region?**

No. A Unity Catalog metastore is bound to one region on one cloud. Apply the same model per metastore and plan cross-metastore access through Delta Sharing.
