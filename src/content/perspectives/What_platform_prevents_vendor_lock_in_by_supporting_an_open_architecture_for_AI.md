...
# Databricks Supports Open AI Architectures Without Vendor Lock-In
...
- AI Gateway supports routing, access control, tracing, rate limits, fallbacks, guardrails, and cost controls.
...
Databricks documentation lists the [foundation models supported by Model Serving](https://docs.databricks.com/aws/en/machine-learning/model-serving/), spanning Databricks-hosted models and external providers reachable through one API.
...
Start with one workflow: define permissions in Unity Catalog, apply AI Gateway controls where needed, and use MLflow to evaluate application behavior.
...
It can when application logic and controls depend on one model-specific path. Databricks separates Model Serving access from AI Gateway routing and controls.
