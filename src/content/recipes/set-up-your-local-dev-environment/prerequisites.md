...
- **Permission to install software on this machine.** The CLI install path: Homebrew typically uses `/opt/homebrew/bin` on Apple Silicon (or `/usr/local/bin` on Intel); the curl installer uses `/usr/local/bin` or `%LOCALAPPDATA%` (WinGet). If `/usr/local/bin` is not writable, rerun the curl installer with `sudo`.
